...
def decision_node(state: AgentState) -> dict:
    ...
    github_username = (state.get("github_username") or "").strip()
    leetcode_username = (state.get("leetcode_username") or "").strip()
    ...
    # Also treat it as career-related when the client provided GitHub/LeetCode usernames,
    # so the tools run even if the user's message doesn't explicitly mention "github"/"leetcode".
    is_career = (
        any(w in query for w in career_keywords)
        or bool(target_company)
        or bool(target_role)
        or bool(github_username)
        or bool(leetcode_username)
    )
    ...
    if is_career:
        ...
        # Force enable tools when usernames are provided from the client.
        # This is the intended UX for the AiDashboard: user enters usernames, then sends a message.
        if github_username:
            use_github = True
